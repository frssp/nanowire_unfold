...
def readPROCAR_phase(file_name):
    """ read PROCAR file of VASP 
        p orbitals are ordered {px, py, pz}
    """
    ...
    def _parse_proj_abs(line, n_orbits):
        ...
        if match is not None:
            ...
            proj = [float(item) for item in proj.split()]
            # swap py, pz, px -> px, py, pz
            proj[1:4] = proj[3:4] + proj[1:3]
            return int(ion_i) - 1, proj
        else:
            return None
...
def readPOSACR(fileName='POSCAR', rtspecies=False):
...
def readCONTCAR(fileName='CONTCAR', rtspecies=False):
    ...
    if rtspecies==True:
        ...
        return [latConst, latticeVecs, atomSetDirect, dynamics_list]
...
def readKPOINTS(fileName='KPOINTS'):
    """for MP input based band calc
    """
    f = open(fileName)
    buffer = f.readlines()
    f.close()
    ...
    for kpt_i, line in enumerate(buffer[3:]):
        line = line.split()
        kpt = [float(pt) for pt in line[:3]]
        if len(line) > 4:
            label = line[4]
        else:
            label = ''
...
if __name__ == '__main__':
    path = '/home/users/nwan/cj01/00_NW/R4/00_pristine/optic'
